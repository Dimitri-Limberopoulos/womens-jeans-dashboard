...
DASHBOARD_IN = os.path.join(BASE, "cross_retailer_dashboard_clean.html")
...
def classify_wash(color_str):
    ...
    pattern_kw = ["print", "pattern", "stripe", "floral", "camo", "plaid",
                   ...
                   "polka", "check", "gingham", "leopard", "animal"]
    for kw in pattern_kw:
        if kw in c:
    ...
    # White/Cream
    white_kw = ["white", "cream", "ivory", "ecru", "bone", "vanilla", "natural",
    ...
    # Grey
    grey_kw = ["grey", "gray", "silver", "slate", "pewter", "graphite", "ash",
               "smoke", "stone", "cement", "concrete", "fog", "mist", "moon",
               "titanium", "nickel", "steel", "heather", "dove"]
    for kw in grey_kw:
        if kw in c:
            return "Grey"

    # Light Wash
    light_kw = ["light wash", "light", "lt wash", "lt ", "lite", "bleach", "faded",
                "pale blue", "sky", "baby blue", "ice blue", "cloud", "acid"]
    for kw in light_kw:
        if kw in c:
            return "Light Wash"

    # Dark Wash
    dark_kw = ["dark wash", "dark", "dk wash", "deep", "indigo", "rinse",
               "night", "midnight", "navy", "ink"]
    for kw in dark_kw:
        if kw in c:
            return "Dark Wash"

    # Medium Wash
    med_kw = ["medium wash", "medium", "med wash", "med ", "mid wash", "vintage",
              "classic", "denim", "destroy", "distress", "ocean", "sea",
              "water", "authentic", "heritage", "traditional", "standard",
              "regular wash", "blue"]
    for kw in med_kw:
        if kw in c:
            return "Medium Wash"
    ...
    # Pink/Red
    pink_kw = ["pink", "red", "rose", "coral", "berry", "blush", "wine",
    ...
    # Brown/Earth
    brown_kw = ["brown", "tan", "khaki", "camel", "earth", "rust",
    ...
    # This catches proprietary wash names (city names like "Fortress", "Chester Wash", etc.)
    if "wash" in c:
        return "Medium Wash"
...
# (Stretch analysis removed per user request)
...
def main():
    ...
    def reclassify_and_collapse_in_raw(raw_str):
        ...
        for entry in entries:
            # Reclassify Other colors
            if entry.get("w") == "Other" and entry.get("c"):
    ...
    # Make coverage the default page (hide overview initially)
    html = html.replace(
    ...
    # Add Coverage + Insights buttons to sidebar
    html = html.replace(
    ...
    # Add coverage + insights nav sections
    html = html.replace(
        '  <div id="nav-overview"',
